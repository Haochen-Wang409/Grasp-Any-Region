...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Inference of Grasp Any Region models on MDVP-Bench."
    )

    parser.add_argument(
        "--model_name_or_path",
        help="HF model name or path",
        default="Haochen-Wang409/GAR-1B",
    )
    parser.add_argument(
        "--cache_name",
    ...
    parser.add_argument(
        ...
        default="evaluation/MDVP-Bench/annotations/mdvp_caption_mask.json",
    )
    parser.add_argument(
        "--image_folder",
        help="the folder of images",
        default="evaluation/MDVP-Bench/data",
    )
    parser.add_argument(
        "--max_num_tiles",
        help="maximum number of tiles for AnyRes",
        type=int,
        default=8,
    )
    parser.add_argument(
...
def main():
    ...
    for idx, item in enumerate(tqdm(data)):
        ...
        mask = mask_utils.decode(item["mask_rle"])
    ...
    with open(f"evaluation/MDVP-Bench/model_outputs/{cache_name}.json", "w") as file:
